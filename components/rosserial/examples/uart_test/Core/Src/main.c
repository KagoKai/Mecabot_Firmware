...
/* Private variables ---------------------------------------------------------*/
UART_HandleTypeDef huart_rosserial;

/* USER CODE BEGIN PV */
uint8_t rx_buffer[8];
uint8_t tx_buffer[8] = "Hello !!";
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart_rosserial, rx_buffer, 8);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  HAL_UART_Transmit(&huart_rosserial, tx_buffer, 8, 500);
	  HAL_Delay(1000);
  }
  /* USER CODE END 3 */
}
...
{
 if(huart->Instance == huart_rosserial.Instance)
 {
   HAL_UART_Transmit(&huart_rosserial, rx_buffer, 8, 500);
   HAL_UART_Receive_IT(&huart_rosserial, rx_buffer, 8);
 }
}
/* USER CODE END 4 */
